...
export const COMPOSE_PIP_WIDTH = 420;
export const COMPOSE_PIP_HEIGHT = 720;

export function isDocumentPipSupported() {
  return typeof window !== "undefined" && "documentPictureInPicture" in window;
}

export async function openComposePipWindow() {
  if (!isDocumentPipSupported()) {
    throw new Error("Document Picture-in-Picture is not supported.");
  }

  const pipWindow = await window.documentPictureInPicture!.requestWindow({
    width: COMPOSE_PIP_WIDTH,
    height: COMPOSE_PIP_HEIGHT,
...
export function watchPipClosed(pipWindow: Window, onClosed: () => void) {
